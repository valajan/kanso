...
const singleRegression = [
  { metric: 'lcp', current: 3200, threshold: 2500, reference: 2400, delta: 33.3 },
];

const multiRegressions = [
  { metric: 'lcp', current: 3200, threshold: 2500, reference: 2400, delta: 33.3 },
  { metric: 'fcp', current: 2800, threshold: 2000, reference: 1500, delta: 86.7 },
];
...
test('prompt embeds the filtered diff and lists anchorable lines', () => {
  const prompt = buildPrompt({
    regressions: [{ metric: 'tbt', current: 500, threshold: 300, reference: 200, delta: 150 }],
    diff: sampleDiff,
  });
...
test('prompt requests JSON output with summary + comments schema', () => {
  const prompt = buildPrompt({
    regressions: [{ metric: 'performance', current: 60, threshold: 80, reference: 90, delta: 33.3 }],
    diff: sampleDiff,
  });
...
test('prompt instructs the model not to invent file paths or line numbers', () => {
  const prompt = buildPrompt({
    regressions: [{ metric: 'cls', current: 0.3, threshold: 0.1, reference: 0.05, delta: 500 }],
    diff: sampleDiff,
  });
...
test('prompt warns the model when a file has no anchorable lines', () => {
  const prompt = buildPrompt({
    regressions: [{ metric: 'fcp', current: 3000, threshold: null, reference: null, delta: 50 }],
