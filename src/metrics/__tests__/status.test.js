...
const lcp = getMetric('lcp');                 // lower is better,  good 2.5, poor 4.0
...
test('buildStatus classifies a lower-is-better metric', () => {
  assert.equal(buildStatus(2.0, null, lcp), 'pass');
  assert.equal(buildStatus(3.0, null, lcp), 'warn');
  assert.equal(buildStatus(5.0, null, lcp), 'fail');
});
...
test('evaluateStatuses returns a status for every metric', () => {
  const statuses = evaluateStatuses(
    { performance: 95, lcp: 2.0, tbt: 100, cls: 0.05, fcp: 1.2 },
    {}
  );
